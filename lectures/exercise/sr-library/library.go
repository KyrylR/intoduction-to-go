...
type BookEntry struct {
	...
	loaned int // total books loaned out
}
...
func printMember(library *Library) {
...
func printLibraryBooks(library *Library) {
	...
	for title, book := range library.books {
		fmt.Println(title, "/ total:", book.total, "/ loaned:", book.loaned)
	}
	fmt.Println()
}
...
func checkoutBook(library *Library, title Title, member *Member) bool {
	...
	if !found {
		fmt.Println("Book not party of library")
		return false
	}
	if book.loaned == book.total {
	...
	member.books[title] = LendAudit{checkOut: time.Now(), checkIn: time.Time{}}
	return true
}
...
func returnBook(library *Library, title Title, member *Member) bool {
	...
	if !audit.checkIn.IsZero() {
		fmt.Println("Member has already returned a book")
		return false
	}
